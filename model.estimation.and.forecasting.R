# CPG STATISTICAL RISK ASSESSMENT 2014
# MODEL ESTIMATION AND APPLICATION
# Jay Ulfelder

# NOTE: This script assumes that you have set the working directory to a location that includes all of the called
# data files and scripts. The R scripts that produce each of the input data files from the source material can also
...
out <- paste("statrisk2014v2", "merge", as.character(Sys.Date()), "csv", sep = ".")
...
# Restrict cross validation to years with observed outcomes
...
png(file = "cpg.statrisk2014v2.val.auc.by.fold.png",
...
png(file = "cpg.statrisk2014v2.val.roc.by.model.png",
     width=12, height=12, units='cm', bg='white', res=150)
...
# Write out in-sample responses for later inspection if desired
insamp <- subset(df, select=c(country, sftgcode, iso3code, year, start.1, mean.p, harff.p, threat.p, rf.p))
write.csv(insamp, file = "insample.csv", row.names = FALSE)
...
# Save results with inputs as .csv
write.csv(newcast, "outofsample.csv", row.names = FALSE)
...
mapname <- paste("c:/users/jay/documents/ushmm/statrisk2014v2/figs/heatmap", datestring, "png", sep = ".")
...
date <- as.Date(Sys.Date())
datestring <- paste(substr(date,1,4), substr(date,6,7), substr(date,9,10), sep="")
mapname <- paste("c:/users/jay/documents/ushmm/statrisk2014v2/figs/scaledmap", datestring, "png", sep = ".")
...
date <- as.Date(Sys.Date())
datestring <- paste(substr(date,1,4), substr(date,6,7), substr(date,9,10), sep="")
dotname <- paste("c:/users/jay/documents/ushmm/statrisk2014v2/figs/dotplot", datestring, "png", sep = ".")
