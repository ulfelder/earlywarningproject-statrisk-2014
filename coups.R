...
require(XLConnect)
csp <- readWorksheetFromFile("cspcoupslist2013.xls", sheet=1)
...
source("countryyear.rack.maker.r")
rack <- subset(rack, year >= 1946 & year <= 2013, select=c(country, sftgcode, year))
...
source("c:/users/jay/documents/ushmm/statrisk2014v2/r/countryyear.rack.maker.r")
rack <- subset(rack, select=c(country, sftgcode, year))
...
write.csv(meld, "c:/users/jay/documents/ushmm/statrisk2014v2/outdata/coups.csv", row.names = FALSE)
